"""PII data v2 = v1 builder + LABEL PROPAGATION (see propagate()).
...
def propagate(text, spans):
    """The generator wraps a value once ('⟦PERSON:Sarah Connor⟧') and later writes 'Thanks, Sarah' untagged.
    v1 learned from that that bare first names are not PII (Privacy-Bench recall fell). Tag every other
    occurrence of each tagged value, and every standalone occurrence of each capitalized token of a tagged name."""
    taken = [False] * len(text)
    for s, e, _ in spans:
        for i in range(s, e): taken[i] = True
    extra = []
    items = {(text[s:e], l) for s, e, l in spans}
    for s, e, l in spans:
        if l in NAME_TYPES:
            for tok in re.findall(r"\w+", text[s:e]):
                if len(tok) >= 2 and tok[:1].isupper():
                    items.add((tok, l))
    for val, l in sorted(items, key=lambda x: -len(x[0])):
        if len(val.strip()) < 2:
            continue
        for m in re.finditer(r"(?<!\w)" + re.escape(val) + r"(?!\w)", text):
            a, b = m.start(), m.end()
            if not any(taken[a:b]):
                extra.append([a, b, l])
                for i in range(a, b): taken[i] = True
    return sorted(spans + extra), len(extra)


tr, ev, stats = [], [], dict(docs=0, neg=0, relabel=0, propagated=0)
for f in sorted(glob.glob(os.environ["GEN"])):
    for line in open(f):
        ...
        for s, e, l in r["spans"]:
            ...
            spans.append([s, e, l])
        spans, n_extra = propagate(text, spans); stats["propagated"] += n_extra
